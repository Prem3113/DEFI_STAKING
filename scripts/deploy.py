...
def add_allowed_tokens(token_farm, dict_allowed_tokes, account):
    for token in dict_allowed_tokes:
        token_tx = token_farm.addAllowedTokens(token.address, {"from": account})
        token_tx.wait(1)
        set_tx = token_farm.setPriceFeedContract(
            token.address, dict_allowed_tokes[token], {"from": account}
        )
        set_tx.wait(1)
        return token_farm
